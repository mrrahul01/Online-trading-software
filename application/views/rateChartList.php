<?php
  ...
  if($carrierRateChartInfo != null)
  {
    ...
    echo form_open(base_url().'alveron/rateChartTrafficTypeEdit',array('id' => 'editForm','style'=>'display:none;'));
  ...
  if($rateChartListView)
  {
    ...
    foreach($rateChartListView as $list)
      ...
      if($updateTrafficTypeAccess == 1)
      {
        $url = base_url().'alveron/rateChartTrafficTypeEdit/'.$list['carrierId'].'/'.$list['rateUploadDate'].'/';
      ...
      if($deleteRateChartAccess == 1)
      {
        $url = base_url().'alveron/rateChartDelete/'.$list['carrierId'].'/'.$list['rateUploadDate'].'/';
        $url .= $searchOptionSelected.'/'.$carrierTypeIdSelected.'/'.$trafficTypeIdSelected.'/'.$carrierNameIdSelected;
