...
<?php
  ...
  // inline search option ends  
    if($carrierListSelect ) // check whether carrier/partner is available
    {
      ...
      echo form_open(base_url().'alveron/partnerSpecialRateChart',array('id' => 'rateChartSearchForm'));
      echo form_fieldset('Top/Push Chart Search');
    ...
    if($partnerRateChartView)
    {
       foreach($carrierInfo as $list)
      {
        if($list['carrierType'] == 'Vendor')
          ...
          $chartName = 'Push';
        $tableCaption = $chartName.' Chart of ['.$list['carrierType'].'] '.strtoupper($list['carrierName']).' - '.strtoupper($list['trafficType']);
