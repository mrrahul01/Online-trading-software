...
<?php
  ...
  if($operation == 0)
  {
    ...
    if($carrierTypeListSelect ) // check whether carrier/partner types are available
    {
      ...
      {
        echo form_open(base_url().'alveron/partnerRateHistory',array('id' => 'partnerRateHistorySearchForm'));
 ...
 elseif($operation == 1)
 {
  ...
  echo '<div id="partnerRateHistoryTable">';
    ...
    {
      $tableCaption = 'Rate History Report of ['.$list['carrierType'].'] '.strtoupper($list['carrierName']).' - '.strtoupper($list['trafficType']);
    ...
    if($partnerRateHistoryView)
    {
      ...
      foreach ($dateList as $date) 
      {
        ...
        array_push($headerArray,$date['startDateTime']);
